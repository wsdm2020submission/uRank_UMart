...
def convert(tfrecords_folder, file_type, fold):
    ...
    for group in group_features:
        ...
        # remove queries with all 0 labels to avoid different NDCG setting confusions
        if np.amax(label_array) < 1:     
            continue        
        # The following setting keeps test unchanged
...
def main():
    tfrecords_folders = ['OHSUMED']# 'OHSUMED', 'MQ2007', 'MSLR-WEB10K', 'MSLR-WEB30K'
    folds = 5
